...
export const Project: React.FunctionComponent<InterfaceProps> = ({
  ...
  const [template, setTemplate] = useState<string>('sword');
  ...
  const createProject = async () => {
    ...
    try {
      const path = 'browser/iota/' + projectName;
  ...
  const getProjectList = async () => {
    try {
      const list = await client?.fileManager.readdir('browser/iota/');
  ...
  const isExists = async (dir: string) => {
    try {
      const read: object = await client.fileManager.readdir('browser/iota/' + dir);
  ...
  const createTemplate = async () => {
    ...
    if (await isExists(template)) {
      await client.terminal.log({
        type: 'error',
        value:
          'The folder "iota/' + template + '" already exists. Please delete the existing project.',
      });
      return;
    }
    ...
    const res = await axios.request({
      method: 'GET',
      url:
        `${COMPILER_API_ENDPOINT}/s3Proxy?bucket=code-template&fileKey=iota/` + template + '.zip',
    ...
    try {
      Object.keys(zip.files).map(async (key) => {
        ...
        if (zip.files[key].dir) {
          await client?.fileManager.mkdir('browser/iota/' + key);
        } else if (!key.startsWith('_') && key !== template + '/.DS_Store') {
          content = await zip.file(key)?.async('string');
          await client?.fileManager.writeFile('browser/iota/' + key, content);
        }
      });
      await wrappedGetList();
